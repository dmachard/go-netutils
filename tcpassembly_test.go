...
func Test_TcpAssembly(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "DNS UDP",
			pcapFile:  "./pcap/dnsdump_udp.pcap",
			nbPackets: 33,
		},

		{
			name:      "DNS TCP",
			pcapFile:  "./pcap/dnsdump_tcp.pcap",
			nbPackets: 10,
		},

		{
			name:      "DNS UDP+TCP",
			pcapFile:  "./pcap/dnsdump_udp+tcp.pcap",
			nbPackets: 4,
		},

		{
			name:      "DNS UDP Truncated + TCP fragmented",
			pcapFile:  "./pcap/dnsdump_udp_truncated+tcp_fragmented.pcap",
			nbPackets: 4,
		},

		{
			name:      "DNS TCP FASTOPEN",
			pcapFile:  "./pcap/dnsdump_tcp_fastopen.pcap",
			nbPackets: 8,
		},
	}
